$(function () {
    ...
    function populateTable($table, data, template) {
        ...
        if (!data.length) {
            $table.html(`<tr><td colspan="6" style="text-align: center">Você não possui nenhum agendamento</td>`);
            return;
        }
    ...
    function getScheduleRowTemplate(schedule) {
        return `
            <tr>
                ...
                ${schedule.message ? `<td class="user-message">${schedule.message}</td>` : `<td>---</td>`}
                <td>${getScheduleStatus(schedule.status)}</td>
                <td>${schedule.barber_name}</td>
            </tr>`;
    }
