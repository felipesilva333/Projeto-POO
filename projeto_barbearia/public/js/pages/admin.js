$(function () {
    ...
    function getBarberRowTemplate(barber) {
        const $tr = $("<tr>");

        const $td1 = $("<td>").attr('data-name', '').text(barber.name);
        const $td2 = $("<td>").append(
            $("<div>").addClass('actions').append(
                $("<button>").attr({ 'data-edit-table': 'barber', 'data-id': barber.id }).append(
                    $("<i>").addClass('bi bi-pencil')
                ),
                $("<button>").attr({ 'data-delete-url': `${url}barber/delete/${barber.id}`, 'type': 'button' }).append(
                    $("<i>").addClass('bi bi-trash')
                )
            )
        );
    ...
    function getScheduleRowTemplate(schedule) {
        ...
        const $td4 = $("<td>").text(schedule.barber_name);
        const $td5 = $("<td>").text(schedule.service);
        const $td6 = $("<td>").text(formatToPrice(schedule.service_price));
        const $td7 = $("<td>").text(formatDate(schedule.datetime));
        const $td8 = $("<td>").addClass("user-message").text(schedule.message ? schedule.message : "---");
        const $td9 = $("<td>").addClass('status-select').append(
    ...
    $('#form-contact-info, #form-service, #form-unavailable-datetime, #form-barber').submit(async function (e) {
        e.preventDefault();
            ...
            if (jsonResponse.status == 200) {
                ...
                switch ($(this).attr('id')) {
                    ...
                    case 'form-unavailable-datetime':
                        ...
                        break;
                    case 'form-barber':
                        updateTable($("[data-table-barbers]"), 'barber/get_all', getBarberRowTemplate);
                        break;
                }
    ...
    $('.admin-item-section').on('click', '[data-edit-table]', function () {
        ...
        switch (table) {
            ...
            case 'barber':
                const tdBarberName = $(this).parents(".actions").parents("td").siblings("td[data-name]").text();
                $("#barber_name").val(tdBarberName);
                $("#barber_id").val(id);
                break;
    ...
    updateTable($("[data-table-barbers]"), 'barber/get_all', getBarberRowTemplate);
    updateTable($("[data-table-unavailable-datetime]"), 'unavailable_datetime/get_all', getUnavailableDatetimeRowTemplate);
    updateTable($("[data-table-contact]"), 'contact_info/get_all', getContactRowTemplate);
